...
OOD_THRESH=0.8
SAM_MODEL='h'
OUTPUT_FOLDER='PINEAPPLES2_5mts_nosplits'
...
BETAS_NUMBERS=('0' '1' '2' '3')
MAHALANOBIS_MODE=('normal' 'regularization')
DIMENSION_VALUES=('4' '8' '16' '32' '64')
DIMENSION_REDUCTION=('none' 'svd')

for TRAIN_VALID_NUM in "${TRAINING_VALIDATION_NUMBER[@]}"; do
    IFS=',' read -r TRAIN_NUM VAL_NUM <<< "${TRAIN_VALID_NUM}"
    for SEED in "${SEEDS_NUMBERS[@]}"; do
        for MAHALANOBIS in "${MAHALANOBIS_MODE[@]}"; do
            if [ "${MAHALANOBIS}" == "normal" ]; then
                BETAS_NUMBERS=('0')
            fi
            for BETA in "${BETAS_NUMBERS[@]}"; do
                for DIM_RED in "${DIMENSION_REDUCTION[@]}"; do
                    if [ "${red}" == "none" ]; then
                        DIMENSION_VALUES=('0')
                    fi
                    for DIM in "${DIMENSION_VALUES[@]}"; do
                        python methods.py \
                            --root ${DATASET_PATH} \
